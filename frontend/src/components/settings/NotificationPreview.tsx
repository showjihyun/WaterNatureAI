  ...
  return (
    <div className="rounded-xl border border-surface-border bg-surface-card p-6 shadow-sm">
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-base font-semibold text-ink">알림 미리보기</h2>
        <button
          onClick={() => refetch()}
          disabled={isFetching}
      ...
      {data && (data.min_score != null || (data.excluded_sources?.length ?? 0) > 0) && (
        <div className="mb-4 flex flex-wrap items-center gap-1.5 text-xs">
          <span className="text-ink-400">적용 규칙</span>
          {data.min_score != null && (
            <span className="rounded-full bg-primary-50 dark:bg-primary-500/15 px-2 py-0.5 font-medium text-primary-700 dark:text-primary-300 ring-1 ring-inset ring-primary-600/10">
              적합도 {data.min_score}점 이상
            </span>
          )}
          {(data.excluded_sources ?? []).map((s) => (
            <span key={s} className="rounded-full bg-surface-muted px-2 py-0.5 text-ink-400">
              {SOURCE_LABEL[s] ?? s} 제외
            </span>
          ))}
        </div>
        ...
        <>
          ...
          <div className="rounded-xl p-4" style={{ backgroundColor: KAKAO_CHAT_BG }}>
            <div className="mx-auto max-w-sm">
              <div className="mb-1.5 flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-ink-700">WaterNature 알림</span>
              </div>

              <div className="overflow-hidden rounded-2xl rounded-tl-md bg-surface-card shadow-sm">
                <div
                ...
                <div className="px-4 py-3">
                  <p className="mb-2.5 text-sm font-semibold leading-snug text-ink">
                    ...
                    {data.items.map((it, i) => (
                      ...
                      >
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-ink-400">
                          ...
                          <span className="ml-auto rounded border border-surface-border bg-surface-card px-1.5 py-0.5 text-[10px] text-ink-400">
                  ...
                  <div className="mt-3 rounded-lg bg-surface-muted py-2 text-center text-[13px] font-medium text-ink-700">
                    오늘의 추천 전체 보기 →
                  </div>
                </div>
              </div>
          ...
          <div className="mt-4">
            {data.would_send ? (
              ...
              <div className="rounded-lg border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/15 px-3 py-2.5">
