...
export function DocumentUploadCard({ currentFilename }: { currentFilename?: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-surface-border bg-surface-card p-6 shadow-sm">
      <h2 className="text-base font-semibold text-gray-900 mb-1">회사소개서</h2>
      ...
      {currentFilename ? (
        ...
        <p className="mb-3 text-xs text-gray-400">아직 업로드한 회사소개서가 없습니다.</p>
      )}

      <label
        className={`flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-surface-border px-4 py-3 text-sm font-medium transition-colors hover:border-primary-300 hover:bg-primary-50/40 ${
      ...
      >
        ...
        {uploading ? (
          <>
            <svg className="h-4 w-4 animate-spin text-primary-600" fill="none" viewBox="0 0 24 24">
          ...
          <>
            ...
            <span className="text-xs text-gray-400">· 최대 20MB</span>
          </>
        )}
      </label>

      {error && <p className="mt-2 text-xs text-red-600">{error}</p>}
      {done && <p className="mt-2 text-xs text-green-700">{done}</p>}
    </div>
  );
}
