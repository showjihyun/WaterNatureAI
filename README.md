...
> **인터넷에 한 번 올려두면** 아래 링크만 열어 **누구나 설치 없이** 체험할 수 있어요. (배포 후 주소를 채워 넣으세요.)

| | 주소 |
|---|---|
| 🟨 **체험용 데모** | `https://여기에-내-주소.vercel.app` ← 배포 후 이 주소를 교체 |

> 직접 내 컴퓨터에서 돌려보려면 아래 **1~3번**을, **인터넷에 올리는 방법**은 **[☁️ 인터넷에 올리기(배포 가이드)](DEPLOY.md)** 를 참고하세요.

---

## 🧭 이게 무엇인가요? (1분)

- **회사 정보를 입력하면**(**표준산업분류(KSIC) 업종**·기술·실적·인증 등) AI가 회사를 이해하고,
- **여러 공공 사이트의 공고**(나라장터·K-Startup·NTIS 등)를 매일 자동으로 모아, **표준 업종으로 자동 분류**하고,
...
**② 공고 탐색 — 출처·지역·유형·업종·예산·마감·적합도로 필터** _(샘플 데이터)_
...
| **공고 탐색** | 전체 공고를 **출처·지역·유형·업종·예산·마감·적합도**로 필터링·정렬. (**유형** = 물품·용역·공사·정부지원 / **업종** = 표준산업분류 13개 대분류) **키워드 워치**(관심 키워드 저장)와 **낙찰 결과**(누가 얼마에 따냈는지) 탭 |
| **내 공고** | **관심**으로 저장한 공고, **진행 관리**(검토중→준비중→제출→완료 단계로 추적) |
| **설정** | 알림 받을 시간·조건, 우리 회사 **수행 역량**(표준 업종·기술수준·감당 예산·수행 유형), 구독/결제, (운영자) AI 공급자·카카오 발신 설정 |
