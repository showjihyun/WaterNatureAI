"""통합 테스트: KStartupCollector.run() — UPSERT / 변경감지 / embed enqueue.
...
def _make_item(
    sn: str = "KS2026T001",
    title: str = "테스트 K-Startup 공고",
    end_dt: str = "20991231",
    agency: str = "창업진흥원",
) -> dict:
    """테스트용 K-Startup raw item."""
    return {
        "pbancSn": sn,
        "biz_pbanc_nm": title,
        "pbanc_ntrp_nm": agency,
        "supt_biz_clsfc": "예비창업",
        "pbanc_rcpt_bgng_dt": "20260601",
        "pbanc_rcpt_end_dt": end_dt,
        "supt_regin": "전국",
        "detl_pg_url": f"https://www.k-startup.go.kr/web/contents/bizpbanc-ongoing.do?pbancSn={sn}",
    }


def _build_collector(items: list[dict], session_factory: Callable) -> KStartupCollector:
    client = _FixedPageClient([items, []])
    return KStartupCollector(client=client, session_factory=session_factory)  # type: ignore[arg-type]
...
class TestKStartupUpsert:
    def _ensure_state(self, db: Session) -> None:
        if not db.get(SourceIngestionState, "kstartup"):
            db.add(SourceIngestionState(source="kstartup"))
            db.flush()
    ...
    def test_new_insert_calls_embed_not_enrich(self, db_session, session_factory):
        ...
        opp = db_session.scalar(
            select(Opportunity).where(Opportunity.source_uid == "KS_T001")
        )
        assert opp is not None
        assert opp.source == "kstartup"
        assert opp.title == "테스트 K-Startup 공고"
        assert opp.agency == "창업진흥원"
    ...
    def test_no_change_on_same_run(self, db_session, session_factory):
        ...
        assert embed.call_count == first_count  # 추가 enqueue 없음

        opp = db_session.scalar(
            select(Opportunity).where(Opportunity.source_uid == "KS_T002")
        )
    ...
    def test_field_change_triggers_update_and_embed(self, db_session, session_factory):
        ...
        self._ensure_state(db_session)
        v1 = _make_item(sn="KS_T003", title="원래 공고명")
        ...
        with patch("app.services.embedding.tasks.embed_opportunity.delay") as embed, \
            ...
            v2 = _make_item(sn="KS_T003", title="변경된 공고명")
            c2 = _build_collector([v2], session_factory)
            with patch.object(c2, "_window", return_value=_recent_window()):
                c2.run()

        assert embed.call_count == 2  # 신규 + 변경 → 2회

        opp = db_session.scalar(
            select(Opportunity).where(Opportunity.source_uid == "KS_T003")
        )
    ...
    def test_state_updated_on_success(self, db_session, session_factory):
        ...
        state = db_session.get(SourceIngestionState, "kstartup")
        assert state.last_status == "success"
        assert state.last_success_at is not None
        assert state.collected_count >= 1

    def test_no_deadline_status_unknown(self, db_session, session_factory):
        """마감일 없음 → status=unknown, deadline NULL, 레코드 보존."""
        self._ensure_state(db_session)
        item = _make_item(sn="KS_T005", end_dt=None)  # type: ignore[arg-type]
        item["pbanc_rcpt_end_dt"] = None
        ...
        opp = db_session.scalar(
            select(Opportunity).where(Opportunity.source_uid == "KS_T005")
        )
        assert opp is not None
